...
@interface JSAPICategoriesApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation JSAPICategoriesApi

NSString* kJSAPICategoriesApiErrorDomain = @"JSAPICategoriesApiErrorDomain";
NSInteger kJSAPICategoriesApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) createCategoryWithCategory: (JSAPICategoryResource*) category
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"JSAPICategoryResource*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPICategoryResource*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) createCategoryTemplateWithTemplate: (JSAPITemplateResource*) template
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/categories/templates"];
...
-(NSURLSessionTask*) deleteCategoryWithId: (NSString*) _id
    ...
    if (_id == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kJSAPICategoriesApiErrorDomain code:kJSAPICategoriesApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/categories/{id}"];
...
-(NSURLSessionTask*) deleteCategoryTemplateWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/categories/templates/{id}"];
...
///
///  @returns JSAPIPageResourceCategoryResource_*
///
-(NSURLSessionTask*) getCategoriesWithFilterSearch: (NSString*) filterSearch
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIPageResourceCategoryResource_*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIPageResourceCategoryResource_*)data, error);
                                }
                            }];
}
