#import "JSAPIRuleEngineRulesApi.h"
...
@implementation JSAPIRuleEngineRulesApi

NSString* kJSAPIRuleEngineRulesApiErrorDomain = @"JSAPIRuleEngineRulesApiErrorDomain";
NSInteger kJSAPIRuleEngineRulesApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) createBRERuleWithBreRule: (JSAPIBreRule*) breRule
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/bre/rules"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"JSAPIBreRule*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIBreRule*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) deleteBRERuleWithId: (NSString*) _id
    ...
    if (_id == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kJSAPIRuleEngineRulesApiErrorDomain code:kJSAPIRuleEngineRulesApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/bre/rules/{id}"];
...
///
///  @returns JSAPIPageResourceBreRule_*
///
-(NSURLSessionTask*) getBRERulesWithFilterName: (NSString*) filterName
    ...
    if (filterAction != nil) {
        queryParams[@"filter_action"] = filterAction;
    }
    if (filterCondition != nil) {
        queryParams[@"filter_condition"] = filterCondition;
    }
    if (size != nil) {
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIPageResourceBreRule_*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIPageResourceBreRule_*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) setBRERuleWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/bre/rules/{id}/enabled"];
