...
@interface JSAPIUsersApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation JSAPIUsersApi

NSString* kJSAPIUsersApiErrorDomain = @"JSAPIUsersApiErrorDomain";
NSInteger kJSAPIUsersApiMissingParamErrorCode = 234513;
...
///  @param order A comma separated list of sorting requirements in priority order, each entry matching PROPERTY_NAME:[ASC|DESC] (optional, default to id:ASC)
///
///  @returns JSAPIPageResourceUserBaseResource_*
///
-(NSURLSessionTask*) getUsersWithFilterDisplayname: (NSString*) filterDisplayname
    ...
    if (filterLastActivity != nil) {
        queryParams[@"filter_last_activity"] = filterLastActivity;
    }
    if (filterIdList != nil) {
        queryParams[@"filter_id_list"] = filterIdList;
    }
    if (filterSearch != nil) {
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIPageResourceUserBaseResource_*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIPageResourceUserBaseResource_*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) registerUserCuentasWithUserResource: (JSAPIUserResource*) userResource
    completionHandler: (void (^)(JSAPIUserResource* output, NSError* error)) handler {
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/users/cuentas"];
