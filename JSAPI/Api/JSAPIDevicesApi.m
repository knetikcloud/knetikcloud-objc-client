...
@interface JSAPIDevicesApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation JSAPIDevicesApi

NSString* kJSAPIDevicesApiErrorDomain = @"JSAPIDevicesApiErrorDomain";
NSInteger kJSAPIDevicesApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) addDeviceUserWithUserResources: (NSArray<JSAPISimpleUserResource>*) userResources
    ...
    if (userResources == nil) {
        NSParameterAssert(userResources);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"userResources"] };
            NSError* error = [NSError errorWithDomain:kJSAPIDevicesApiErrorDomain code:kJSAPIDevicesApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/devices/{id}/users"];
    ...
    return [self.apiClient requestWithPath: resourcePath
                       ...
                       responseContentType: responseContentType
                              responseType: @"JSAPIDeviceResource*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIDeviceResource*)data, error);
                                }
                            }];
}
...
-(NSURLSessionTask*) createDeviceTemplateWithDeviceTemplateResource: (JSAPITemplateResource*) deviceTemplateResource
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/devices/templates"];
...
-(NSURLSessionTask*) deleteDeviceWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/devices/{id}"];
...
-(NSURLSessionTask*) deleteDeviceTemplateWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/devices/templates/{id}"];
...
-(NSURLSessionTask*) deleteDeviceUserWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/devices/{id}/users/{user_id}"];
...
///  @param order A comma separated list of sorting requirements in priority order, each entry matching PROPERTY_NAME:[ASC|DESC] (optional, default to id:ASC)
///
///  @returns JSAPIPageResourceDeviceResource_*
///
-(NSURLSessionTask*) getDevicesWithFilterMake: (NSString*) filterMake
    ...
    NSMutableDictionary* queryParams = [[NSMutableDictionary alloc] init];
    if (filterMake != nil) {
        queryParams[@"filter_make"] = filterMake;
    }
    if (filterModel != nil) {
        queryParams[@"filter_model"] = filterModel;
    }
    if (filterOs != nil) {
        queryParams[@"filter_os"] = filterOs;
    }
    if (filterSerial != nil) {
        queryParams[@"filter_serial"] = filterSerial;
    }
    if (filterType != nil) {
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"JSAPIPageResourceDeviceResource_*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((JSAPIPageResourceDeviceResource_*)data, error);
                                }
                            }];
}
